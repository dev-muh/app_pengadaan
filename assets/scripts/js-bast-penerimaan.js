...
$('#form-add-bast-penerimaan [name="tanggal_bast"]').datepicker();
$('#form-add-bast-penerimaan [name="spb"]').select2({
    placeholder: "-- Pilih SPB --",
    width: '100%',
    ajax: {
      url: URL + 'transaksi/get_select2_spb_without_bast',
      dataType: 'json',
      type: 'post',
      delay: 250,
...
$('#form-add-bast-penerimaan').submit(function(event) {
  /* Act on the event */
  event.preventDefault();

    startloading('Menambahkan BAST Penerimaan...');
    
    $.ajax({
        url: URL +   'transaksi/create_bast',
        type: 'POST',
        data: $(this).serialize(),
        dataType: "json",
      })
    .always(function(result) {
        ...
        $.alert({
            ...
            buttons:{
                ok:function(){
                    window.location.href = URL + 'transaksi/bast_penerimaan_brg/view'
                }
            }
        });
    });
...
function get_pdf_bast(item_id){
  window.location.href = URL + 'transaksi/export_pdf_bast?id=' + item_id
